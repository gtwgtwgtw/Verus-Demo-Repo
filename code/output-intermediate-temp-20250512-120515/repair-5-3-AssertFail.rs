...
/****************************************************
 * Helper lemma to prove index_of for first occurrence
 ****************************************************/
proof fn lemma_first_occurrence_index_of(nums: Seq<i32>, i: int)
    requires
        i < nums.len(),
...
{
    // By definition of index_of, i is the smallest j with nums[j] == nums[i].
    // No j < i with nums[j] == nums[i], so index_of(nums[i]) = i.
}
        ...
        decreases nums.len() - i
    {
        if !contains(&res, nums[i]) {
            ...
            proof {
                ...
                lemma_first_occurrence_index_of(nums@, ( i ) as int);
