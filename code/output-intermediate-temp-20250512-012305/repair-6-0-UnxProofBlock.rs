            ...
            ==> nums@.index_of(res@[i]) < nums@.index_of(res@[j]),
{
        ...
        decreases nums.len() - i
    {
        if !contains(&res, nums[i]) {
            res.push(nums[i]);
            proof {
                let new_elem = nums@[i as int];
                let idx = nums@.index_of(new_elem);
                assert(idx >= i);
                // Because new_elem = nums@.index(i), i is an occurrence of new_elem
                // so index_of(new_elem) <= i
                assert(idx <= i);
                assert(idx == i);
...
//                     // so index_of(new_elem) <= i
//                 }
//   None: proof {                     // Because new_elem = nums@.index(i), i is an occurrence of new_elem                     // so index_of(new_elem) <= i                 }
