        ...
        decreases nums.len() - i
    {
        if !contains(&res, nums[i]) {
            ...
            proof {
                ...
                // new_elem not in subrange(0..i)
                // because res@.to_set() matches subrange(0..i).to_set(),
                // and we just checked it's not in res.
                let idx = nums@.index_of(new_elem);
                assert(idx >= i);  // it can't appear before i
                assert(idx <= i);  // it's nums@[i]
                assert(idx == i);
