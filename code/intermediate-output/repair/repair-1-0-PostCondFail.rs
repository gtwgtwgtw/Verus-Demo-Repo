...
//     result
//   at the end of the function body: result
//   failed this postcondition: forall|i: int, j: int|              0 <= i && i < j && j < v@.len()             && v@[i] <= e && v@[j] <= e             ==> result@.index_of(v@[i]) < result@.index_of(v@[j])
