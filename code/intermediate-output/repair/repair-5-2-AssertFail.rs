            ...
            ==> nums@.index_of(res@[i]) < nums@.index_of(res@[j]),
{
        ...
        decreases nums.len() - i
    {
        if !contains(&res, nums[i]) {
            ...
            res.push(nums[i]);
            proof {
                let new_index = old_len;
                assert(nums@.index_of(res@[( new_index ) as int]) == i);
                assert(forall|x: int| 0 <= x < new_index ==> nums@.index_of(res@[x]) < i);
                assert(forall|x: int| 0 <= x < new_index ==> nums@.index_of(res@[x]) < nums@.index_of(res@[( new_index ) as int]));
            }
        }
        i += 1;
        proof {
...
//                 0 <= x && x < y && y < res@.len()
//                 ==> nums@.index_of(res@[x]) < nums@.index_of(res@[y]));
//   assertion failed: forall|x: int, y: int|                 0 <= x && x < y && y < res@.len()                 ==> nums@.index_of(res@[x]) < nums@.index_of(res@[y])
